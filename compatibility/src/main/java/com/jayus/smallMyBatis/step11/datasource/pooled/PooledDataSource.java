...
import com.jayus.smallMyBatis.step11.datasource.unpooled.UnpooledDataSource;
    ...
    protected void pushConnection(PooledConnection connection) throws SQLException {
        synchronized (state) {
            ...
            if (connection.isValid()) {
                // 如果空闲链接小于设定数量 也就是太少时
                if (state.idleConnections.size() < poolMaximumIdleConnection && connection.getConnectionTypeCode() == expectedConnectionTypeCode) {
                    state.accumulatedCheckoutTime += connection.getCheckoutTime();
                    if (!connection.getRealConnection().getAutoCommit()) {
                        connection.getRealConnection().rollback();
                    }
                    // 实例化一个新的 DB 链接 加入到 idle 列表
            ...
            synchronized (state) {
                    ...
                    // 活跃连接数已满
                        ...
                        // 如果 checkout 超时时间不够长 则等待
                            ...
                            try {
                                ...
                                state.accumulatedWaitTime += System.currentTimeMillis();
                ...
                if (conn != null) {
                    if (conn.isValid()) {
                        ...
                        state.requestCount++;
                        state.accumulatedRequestTime += System.currentTimeMillis();
                    } else {
        ...
        synchronized (state) {
            ...
            for (int i = state.activeConnections.size(); i > 0; i--) {
                try {
                    ...
                    Connection realCon = conn.getRealConnection();
                    if (!realCon.getAutoCommit()) {
                        realCon.rollback();
                    }
                    realCon.close();
                } catch (Exception e) {
